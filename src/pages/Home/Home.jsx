...
const Home = () => {
        ...
        const formattedBooks = await Promise.all(
          res.map(async (book) => {
            try {
              ...
              console.error(`Failed to fetch book details for ID: ${book.ebookId}`, error);
              return null;
            }
          })
        );
  ...
  const filterBooks = (books, conditions) => {
    return books.filter((book) => {
      if (conditions.minAmount && book.amount < conditions.minAmount) return false;
  ...
  if (loading) {
    return <div className="text-center py-20 text-xl">Loading, please wait...</div>;
  }
